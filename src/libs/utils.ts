...
export const prefixZero = (n: number): string | number => +n > 9 ? n : `0${n}`
...
export function formatDate(timestamp = Date.now(), format = 'Y-M-D h:m:s') {
    ...
    return format
        .replace('Y', String(dateInfo.Y))
        .replace('M', String(dateInfo.M))
        .replace('D', String(dateInfo.D))
        .replace('W', String(dateInfo.W))
        .replace('h', String(prefixZero(dateInfo.h)))
        .replace('m', String(prefixZero(dateInfo.m)))
        .replace('s', String(prefixZero(dateInfo.s)));
}
...
export function wxJsPay(payParams: any, callBack: any = {}) {
...
export function pageData(page = 2) {
...
export const Storage = {
    ...
    update(key: string, prototypes: { [key: string]: any }, expireTime: null | number = Infinity): void {
