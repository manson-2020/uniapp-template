...
const { beta, release, path, page, authValidityDay } = config,
    ...
    codeHandler: {
        notAuth: async ({ msg: title }: Response): Promise<string | void> => {
            ...
            if (page.auth) {
                ...
                pretreatment.throttle = true;
                await sleep(1.2)
                await uni.reLaunch({ url: page.auth, success: Storage.clear });
            ...
            Storage.set("authInfo", data, authValidityDay);
...
$request.interceptors.request.use<RequestOptions>(
    params => {
        ...
        authInfo && (params.header.token = authInfo.token);
...
$request.interceptors.response.use<UniApp.RequestSuccessCallbackResult>(
    res => {
        ...
        try {
            ...
            uni.showToast({ title: message as string, icon: "none" });
